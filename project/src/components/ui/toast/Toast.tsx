...
const Toast: React.FC<ToastProps> = ({ type, title, message, onClose, duration = 4000 }) => {
  ...
  return (
    <div
      className={`rounded-xl border p-4 shadow-lg w-80 mb-4 flex items-start gap-3 ${variantClasses[type].container} animate-slide-in-right`}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      role="alert"
    >
      <div className={`-mt-0.5 ${variantClasses[type].icon}`}>{variantClasses[type].iconSvg}</div>
      ...
      <button
        onClick={onClose}
        className="ml-2 text-gray-400 hover:text-gray-700 dark:hover:text-white text-lg font-bold"
        aria-label="Fechar"
      >
        ×
      </button>
